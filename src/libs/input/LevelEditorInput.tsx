...
/**
 * The `LevelEditorInput` class is responsible for managing 
 * the level editor's user interaction and keybindings and 
 * provides a way to set and interact with the selected brush.
 * 
 * NOTE: The `LevelEditorInput` class is not meant to be instantiated directly. Instead, it should be created using the `LevelEditor` class.
 * @class
 */
class LevelEditorInput {
    ...
    private add(x: number, y: number): number[] {
        if (this.__editable.value && LevelEditor.brush) {
            const res = this.__renderer.addTexture(this.__clipping.value, "tilesets", LevelEditor.brush.group, LevelEditor.brush.id, x, y);
            if (res.length > 0) {
                this.__renderer.render();
            }
            return res;
    ...
    private remove(x: number, y: number): number[] {
        if (LevelEditor.brush) {
            const res = this.__renderer.removeTexture(this.__clipping.value, "tilesets", LevelEditor.brush.group, LevelEditor.brush.id, x, y);
            if (res.length > 0) {
                this.__renderer.render();
            }
            return res;
    ...
    private handleBrush(event: MouseEvent): void {
        if (!this.ready || !LevelEditor.brush) return;
