...
class RTree {
  ...
  insert = (bounds: Bound, value: any) => {
    const node = this.#chooseLeafNode(bounds);
    node.bounds = this.#adjustBounds(node.bounds, bounds);
    const records = node.children as Record[];
    records.push(new Record(bounds, value));
    if (node.children.length > this.#maxCapacity) {
