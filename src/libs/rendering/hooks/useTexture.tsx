import { clip, inBounds, inBoundsX, inBoundsY } from '../utils/placement';
import { Config, RevisionRecord, Texture, TextureObjectBounds, TextureSources } from '../types';
import { getHeight, getWidth } from '../../../utils/styleProps';
import { Signal, useSignal } from '@preact/signals-react';
import { Sprite, Tile } from '../../object';
import { clearArc } from '../utils/canvas';
...
const useTexture = (ctx: CanvasRenderingContext2D) => {
  ...
  function textureExists(x: number, y: number) {
    return tree.search({
      minX: x,
      minY: y,
      maxX: x,
  ...
  function addTexture(src: string, name: string, clipping: boolean,
    ...
    const [scaledX, scaledY] = clipping
      ? clip(ctx.canvas.height, ctx.canvas.width, x, y, w, h)
      : inBounds(ctx.canvas.height, ctx.canvas.width, x, y, w, h);
    if (textureExists(scaledX, scaledY)) {
      return [];
    }
