  ...
  return (
    <div className="Playback">
      <div className="buttons">
        <button data-cy="animation-repeat" title="repeat" onClick={() => timelineControls.repeat()} style={{
          backgroundColor: `${timelineControls.loop.value
            ? 'var(--accent-color)'
            : 'transparent'}`
        }}>
        ...
        <button data-cy="animation-play-previous-frame" title="play previous frame"
          onClick={() => timelineControls.playFastBackward()}>
          <img alt="animation play previous frame" src={arrowLeftLeftIcon}/>
        </button>
        <button data-cy={`animation-${timelineControls.playing.value
          ? 'stop'
          : 'play'}`} title={timelineControls.playing.value
          ? 'stop'
          : 'play'} onClick={() => (timelineControls.playing.value
          ? timelineControls.stop()
          : timelineControls.play())}>
          <img alt={`animation ${timelineControls.playing.value
            ? 'stop'
            : 'play'}`} src={timelineControls.playing.value
            ? stopIcon
            : playIcon}/>
        </button>
        <button data-cy="animation-play-next-frame" title="play next frame"
          onClick={() => timelineControls.playFastForward()}>
