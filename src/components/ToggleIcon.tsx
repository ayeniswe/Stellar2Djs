...
const ToggleIcon: React.FC<ToggleIconProps> = ({ name, src, fn, title, keyShortcuts, cy }) => {
  ...
  return (
    <button data-cy={cy} id={name} ref={button} className='ToggleIcon'
      aria-keyshortcuts={keyShortcuts} aria-label={`${name} off`} title={title}
      onClick={() => fn(button.current, status.current, icon.current)}>
